...
document.addEventListener("DOMContentLoaded", function() {
    ...
    // Function to update the environment parameters display
    function updateEnvironmentParametersDisplay() {
        ...
        var co2Level = 421; // Set CO2 level to be around 421 ppm
        var humidity = Math.floor(generateRandomNumber(64, 68));
        var calibrate = Math.random() < 0.5 ? "True" : "False"; // Randomly set to True or False
        ...
        var aqi = 20; // AQI value
        var aqiStatus = "Good air"; // AQI status
        ...
        // Check AQI status and update color accordingly
        if (aqi <= 50) {
    ...
    // Initial update of environment parameters display
    updateEnvironmentParametersDisplay();

    // Update environment parameters display every 10 seconds
